...
export default function Catalog() {
    ...
    const [entertainments, setPosts] = useState([])
    const [data, setData] = useState(entertainmentsCatalog);
    ...
    const sorted = entertainments.sort((a , b) => {
        if (sortType != null) {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:5000/entertainment')
            .then(res => {
            ...
            <div className="">
                <div className="">
                    <div className={classes.root}>
                        <DataFetching entertainments={ entertainments } />
                        {entertainments.length === 0 && <span>No records found to display!</span>}
                    </div>
                </div>
            </div>
        </div>
